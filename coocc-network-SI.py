...
df = pd.read_csv("data/parliamentSI.csv")
...
stop_words.update({"tu", "tam", "kjer"})

seed_words = ["umeten", "inteligenca", "umetno"]

with open("data/parlamint-stopwords-SI.txt") as f:
...
# filter on less than N occurrences
filtered_tokens = [token for token in tokens if fdist[token] > 5]
...
nx.write_pajek(G, path=f"ParlamintSI.net",
               encoding='utf-8')
df2.to_csv(f"ParlamintSI.tab", sep="\t",
          index=False)
